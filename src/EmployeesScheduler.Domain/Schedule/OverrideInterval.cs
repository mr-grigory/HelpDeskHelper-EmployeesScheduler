...
public class OverrideInterval
{
    ...
    private OverrideInterval(DateTime startAt, DateTime endAt, WorkState state)
    {
        StartAt = startAt;
        EndAt = endAt;
        State = state;
    ...
    public static Result<OverrideInterval, Error> Create(DateTime startAt, DateTime endAt, WorkState state)
    {
        if (startAt >= endAt)
            return Error.Validation("OverrideInterval.Create", "startAt cannot be greater than endAt");
        
        return new OverrideInterval(startAt, endAt, state);
    }
}